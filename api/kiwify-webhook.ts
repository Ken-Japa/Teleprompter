...
async function kiwifyHandler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (!email || !key) {
      console.warn("[WEBHOOK_SKIP] Missing email or key in payload. Data received:", {
        hasEmail: !!email,
        hasKey: !!key,
    ...
    console.error("[WEBHOOK_ERROR] Critical failure processing Kiwify payload:", {
      error: error instanceof Error ? error.message : error,
      payload: JSON.stringify(payload).substring(0, 500) // Log partial payload for context
    });
