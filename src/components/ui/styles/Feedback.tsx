...
export const StatusBadge = ({ status, label }: { status: string, label: string }) => {
  ...
  return (
    <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded border ${colorClass} tracking-wider transition-all duration-300`}>
      {label}
    </span>
  );
};
...
export const PaywallModal = ({ title, desc, children }: { title: string, desc: string, children: React.ReactNode }) => (
...
export const Watermark = ({ text, theme }: { text: string, theme?: string }) => {
    ...
    return (
        <div className={`absolute bottom-32 left-0 right-0 text-center pointer-events-none z-0 select-none ${color}`}>
            <span className="text-6xl md:text-8xl font-black uppercase tracking-tighter transform -rotate-6 inline-block">
            {text}
            </span>
        </div>
    );
