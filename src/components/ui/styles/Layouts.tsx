...
export const ScreenContainer = React.forwardRef<HTMLDivElement, { children: React.ReactNode, className?: string } & React.HTMLAttributes<HTMLDivElement>>(({ children, className = '', ...props }, ref) => (
...
export const MainContent = ({ children, onMouseMove }: { children: React.ReactNode, onMouseMove?: () => void }) => (
...
export const EditorTextArea = React.forwardRef<HTMLTextAreaElement, { value: string, onChange: (e: React.ChangeEvent<HTMLTextAreaElement>) => void, placeholder: string }>(({ value, onChange, placeholder }, ref) => (
  <textarea
    ref={ref}
    className="flex-1 bg-transparent p-8 md:p-12 text-lg md:text-xl font-mono text-slate-300 resize-none focus:outline-none focus:ring-0 scrollbar-thin scrollbar-thumb-slate-800 placeholder:text-slate-700 leading-relaxed max-w-5xl mx-auto w-full relative z-10 selection:bg-indigo-500/30"
    value={value}
    onChange={onChange}
...
export const QRCodeBox = ({ children, hasId }: { children: React.ReactNode, hasId: boolean }) => (
  <div className={`relative group p-1.5 bg-gradient-to-br from-indigo-500/30 via-purple-500/30 to-pink-500/30 rounded-3xl transition-all duration-700 ${hasId ? 'shadow-[0_0_60px_rgba(99,102,241,0.15)] scale-100' : 'scale-95 opacity-50 grayscale'}`}>
...
export const ToolbarDivider = () => (
  <div className="h-6 w-[1px] bg-white/10 mx-6 shrink-0" />
);

// --- PROMPTER & HUD ---

export const PrompterScrollArea = React.forwardRef<HTMLDivElement, { children: React.ReactNode, className?: string } & React.HTMLAttributes<HTMLDivElement>>(({ children, className = '', style, ...props }, ref) => (
  <div ref={ref} className={`w-full h-full overflow-y-scroll scrollbar-hide relative z-10 ${className}`} style={{ scrollBehavior: 'auto', ...style }} {...props}>
    {children}
  </div>
));
...
export const HudContainer = ({ children, visible }: { children: React.ReactNode, visible: boolean }) => (
  <div className={`fixed bottom-12 left-1/2 -translate-x-1/2 glass-panel pl-6 pr-4 py-3 rounded-full z-50 transition-all duration-500 cubic-bezier(0.16, 1, 0.3, 1) flex items-center space-x-6 ring-1 ring-white/10 shadow-[0_20px_60px_-10px_rgba(0,0,0,0.8)] ${visible ? 'opacity-100 translate-y-0 scale-100 blur-0' : 'opacity-0 translate-y-12 scale-90 blur-lg pointer-events-none'}`}>
    {children}
  </div>
);
...
export const HudGroup = ({ children, label }: { children: React.ReactNode, label?: string }) => (
  <div className="flex flex-col items-center space-y-1.5 border-r border-white/10 last:border-0 pr-6 last:pr-0 group relative">
    ...
    {label && <span className="text-[9px] uppercase text-slate-500 font-bold tracking-[0.2em] group-hover:text-indigo-400 transition-colors absolute -bottom-5 left-1/2 -translate-x-1/2 whitespace-nowrap opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 duration-300">{label}</span>}
  </div>
);
...
export const TouchPad = React.forwardRef<HTMLDivElement, { onTouchStart: React.TouchEventHandler<HTMLDivElement>, onTouchMove: React.TouchEventHandler<HTMLDivElement>, onTouchEnd?: React.TouchEventHandler<HTMLDivElement>, label: string, children: React.ReactNode, className?: string }>(({ onTouchStart, onTouchMove, onTouchEnd, label, children, className = '' }, ref) => (
  <div
    ref={ref}
