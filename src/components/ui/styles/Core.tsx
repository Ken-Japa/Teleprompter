...
export const LogoText = ({ main, sub }: { main: string, sub: string }) => (
  <div className="flex items-center space-x-3 group cursor-pointer select-none">
    ...
    <h1 className="text-xl font-bold text-white tracking-tight flex items-center font-display">
      ...
      <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-violet-300 ml-0.5 tracking-wide font-extrabold filter drop-shadow-[0_0_10px_rgba(139,92,246,0.3)]">{sub}</span>
    </h1>
  </div>
);
...
export const PrimaryButton = ({ onClick, children, className = '', ...props }: { onClick: () => void, children: React.ReactNode, className?: string } & React.ButtonHTMLAttributes<HTMLButtonElement>) => (
  <button 
    onClick={onClick}
...
export const SecondaryButton = ({ onClick, children, className = '', ...props }: { onClick: () => void, children: React.ReactNode, className?: string } & React.ButtonHTMLAttributes<HTMLButtonElement>) => (
  <button 
    onClick={onClick}
...
export const IconButton = ({ onClick, children, active = false, className = '', ...props }: { onClick: () => void, children: React.ReactNode, active?: boolean, className?: string } & React.ButtonHTMLAttributes<HTMLButtonElement>) => (
  <button 
    onClick={onClick} 
...
export const RangeSlider = ({ value, min, max, step = 1, onChange, width = "w-24", ariaLabel }: { value: number, min: number, max: number, step?: number, onChange: (val: number) => void, width?: string, ariaLabel?: string }) => (
...
export const ColorButton = ({ color, onClick, label }: { color: 'red' | 'yellow' | 'green' | 'blue', onClick: () => void, label: string }) => {
    const styles = {
