...
export const FontControl = memo(({ fontSize, setFontSize }: FontControlProps) => {
 ...
 return (
  <S.HudGroup label={t("host.controls.size")}>
   <S.IconButton onClick={() => setFontSize((f) => Math.max(20, f - 4))} aria-label="Decrease Font Size">
   ...
   <S.RangeSlider
    ...
    ariaLabel="Font Size"
   />
   <S.IconButton onClick={() => setFontSize((f) => Math.min(200, f + 4))} aria-label="Increase Font Size">
