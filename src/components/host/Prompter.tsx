import React, { useEffect, useRef, useState, memo, useCallback, useMemo, forwardRef, useImperativeHandle } from 'react';
...
export const Prompter = memo(forwardRef<PrompterHandle, PrompterProps>(({
    text, isPro, status, onExit, setShowPaywall,
    externalState, onStateChange, onScrollUpdate
}, ref) => {
    ...
    // Voice Control
    const { startListening, stopListening, resetVoice, activeSentenceIndex, sentences } = useVoiceControl(text, isPro);
    ...
    const getThemeClass = useCallback(() => {
        switch (theme) {
            case 'paper': return 'bg-white text-slate-900';
            case 'contrast': return 'bg-black text-yellow-400 font-bold';
            case 'matrix': return 'bg-black text-green-500 font-mono';
            case 'cyber': return 'bg-slate-900 text-pink-500 shadow-[inset_0_0_100px_rgba(236,72,153,0.1)]';
            case 'cream': return 'bg-[#fdfbf7] text-[#333]';
    ...
    }, [isPro, isVoiceMode, setShowPaywall, stopListening, startListening, onStateChange, externalState.speed, currentActiveElementRef]);
    ...
    } as React.CSSProperties), [fontSize, margin]);
    ...
    return (
        ...
        >
            {theme === 'matrix' && <div className="absolute inset-0 pointer-events-none opacity-10 bg-[url('https://placehold.co/100x100/000000/00FF00?text=01')]"></div>}
