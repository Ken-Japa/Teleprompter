    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (wrapperRef.current && !wrapperRef.current.contains(event.target as Node)) {
                // If it's mobile, we handle close via backdrop if portal is used, 
                // but for safety we keep this. However, since the portal is outside the ref, 
                // we should be careful not to close when clicking the portal.
                if (!isMobile) {
                    setIsOpen(false);
                }
            }
