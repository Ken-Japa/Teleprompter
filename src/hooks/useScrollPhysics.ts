...
interface PhysicsParams {
    ...
    metricsRef: React.MutableRefObject<{ scrollHeight: number; clientHeight: number }>;
...
export const useScrollPhysics = ({
    ...
    const loop = useCallback((timestamp: number) => {
        ...
        // 2. Auto Scroll
        if (isPlaying && !isUserTouchingRef.current && !isManualScrollingRef.current && Math.abs(momentumRef.current) < 0.5) {
        ...
        // 4. Voice
        if (isVoiceMode && activeSentenceIndex !== -1 && !isPlaying && !isUserTouchingRef.current && !isManualScrollingRef.current) {
            if (activeSentenceIndex !== lastVoiceIndexRef.current) {
                ...
                if (activeEl) {
                    ...
                    targetVoiceScrollRef.current = activeEl.offsetTop - (metrics.clientHeight / 2) + (activeEl.clientHeight / 2);
                }
        ...
        if (!isPlaying && !hasMomentum && !isMoving && !isUserTouchingRef.current && !isManualScrollingRef.current) {
            // Check voice stability
            const voiceStable = !isVoiceMode || (targetVoiceScrollRef.current === null || Math.abs(targetVoiceScrollRef.current - internalScrollPos.current) < 1);
    ...
    const handleRemoteInput: RemoteScrollHandler = useCallback((delta, stop = false, hardStop = false) => {
        if (stop) {
